...
function metaFor(route) {
  ...
  const title = raw.match(/^title:\s*(.*)$/m)?.[1]?.trim().replace(/^"|"$/g, '') ?? null;
...
for (const file of walk(OUT, (n) => n.endsWith('.html'))) {
  ...
  const rel = path.relative(OUT, file).replace(/\.html$/, '').split(path.sep).join('/');
...
fs.writeFileSync(path.join(OUT, 'docs-pages.json'), JSON.stringify(pages, null, 2));
