...
The most common way to create an array is using the ``$arrayCreate`` function. This function takes a piece of text and divides it into elements based on a separator you specify.
...
`$arrayCreate` itself doesn't display any output. We need another function to verify that the array was created correctly.  We'll use ``$arrayLength`` to count the number of elements in the array.
...
Sometimes you might prefer to build an array dynamically, adding elements one at a time. The ``$arrayPush`` function allows you to append new elements to the end of an existing array.
...
While many array functions rely on element indexes, you'll often need to find the index of a specific value within an array.  The ``$arraySearch`` function makes this easy.
