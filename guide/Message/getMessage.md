...
*   `attribute`:  Specifies which piece of information you want to retrieve from the message.  Available attributes are:
    ...
    *   `userID/authorid`: The ID of the user who sent the message.
    *   `description/desc`: (Applicable for embeds only) The description of the embed associated with the message. If the message has no embed or the embed has no description, this will return an empty string.
...
3.  **Getting the embed description (if the message contains an embed):**
    ...
    ```bash
    $getMessage[987654321098765432;123456789012345678;desc]
    ```
