...
The reward that users will receive is 100$ with the [economy command from the community](/Guide/4.template). Since the economy uses the `money` user var to store a user's balance, we will contribute to that var.
