...
* Support for text display in `$modal`, a text that support discord markdown and emojis.
* Support for `status` and `folder` for `$editTrigger` and `$getTrigger`
* New option 'offset' for `$timestamp`, to easily get timestamp relative to value like `$timestamp[yes;+1h]`.
