...
<Callout type="info" title={"How to execute functions"}>
    All functions can be either executed by writing a command in the dashboard,
    or using the built in `!!exec` command.
</Callout>
...
<Discord>
    <Message>!!exec $math[1+1] = $mAtH[1+1] = $MATH[1+1]</Message>
    <Message bot>2 = 2 = 2</Message>
</Discord>
...
<Discord>
    <Message>
        !!exec $title[Math question] $description[What is 2^11? Click to reveal:
        ||$math[2^11]||]
    </Message>
    <Message bot>
        <Embed title="Math question">
...
- <Arg name="Channel ID" default="$channelID" code="true" /> - ID of channel you
  want to retrieve information from.
- <Arg name="Message ID" default="$messageID" code="true" /> - ID of the message
  you want to retrieve information from.
- <Arg name="Option" /> - What kind of information you want to retrieve.
- <Arg name="Additional 1" default="None" /> - Additional argument. Some options
  need these to work properly.
- <Arg name="Additional 2" default="None" /> - Additional argument. Some options
  need these to work properly.
...
<Discord>
    ...
    <Message bot>
        Message content: !!exec Message content:
        $msg[$channelID;$messageID;content]
    </Message>
</Discord>
