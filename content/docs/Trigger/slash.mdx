...
### Sending Message
Next, to send a message with [$interactionReply[message]](/Interaction/interactionReply)\
Here we will send an embed with a title and image using \{title} and \{image} [Curl Message Format](/CodeReferences/ref.message_curl_format):
```cc
$interactionReply[
    {title:Embed Title}
    {image:Embed Image}
...
To get the username `Mido#1234` we will use [$userTag[user id]](/Member/userTag), to specifiy the user we will use `$user_id`:
```
Avatar of $userTag[$user_id]
```

2. In Image to retrieve the user avatar, we will use [$userAvatar[user id]](/Member/userAvatar):
```cc
$userAvatar[$user_id]
```
