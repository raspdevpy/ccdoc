...
- **max:** The maximum value of the range. The behavior of this value depends on whether decimals are allowed:
    - **If `allowDecimals` is `no` (or omitted):** `max` is _inclusive_. The random number will be between `min` and `max`, _including_ `max`.
    - **If `allowDecimals` is `yes`:** `max` is _exclusive_. The random number will be between `min` and `max`, _not including_ `max`.
...
<Discord>
    <Message>!!exec `$random[1;6]`</Message>
