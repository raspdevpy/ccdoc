...
#### Usage
```cc
$sendWebhook[webhookID;webhookToken;message;return message ID (yes/no, optional);username (optional);avatar URL (optional)]
```
...
You can send complex messages with embeds using the [Message Curl Format](/CodeReferences/ref.message_curl_format).
...
**Tags:** <Tag text="webhook"/> <Tag text="message"/>
