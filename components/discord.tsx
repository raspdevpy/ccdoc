...
import { botIcon } from "@/lib/shared";
 ...
 * </DiscordEmbed>
 */
type DiscordEmbedProps = Omit<ComponentProps<typeof RawEmbed>, "children"> & {
    children?: ReactNode;
};

export function DiscordEmbed({ children, ...props }: DiscordEmbedProps) {
    const { resolvedTheme } = useTheme();
    ...
    if (childArray.length === 0) {
        return <RawEmbed lightTheme={resolvedTheme === "light"} {...props} />;
    }
    ...
    return (
        <RawEmbed lightTheme={resolvedTheme === "light"} {...props}>
            {isPlainContent ? (
...
export function DiscordMessageUser({ children, ...props }: SimpleMessageProps) {
...
export function DiscordMessageBot({ children, ...props }: SimpleMessageProps) {
    const lightTheme = useTheme().resolvedTheme === "light";
